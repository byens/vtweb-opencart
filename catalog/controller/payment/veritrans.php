	...
	protected function index() {
		...
		if ($this->cart->hasShipping()){
			...
			$shipping_fee= array("COMMODITY_ID" => "0",
					"COMMODITY_PRICE" => number_format($shipping_cost,0,'',''),
					"COMMODITY_QTY" => 1,
		...
		if ($veritrans->gross_amount!=$productprice){
			...
			$fee= array("COMMODITY_ID" => "aa",
					...
					"COMMODITY_QTY" => 1,
					"COMMODITY_NAME1" => "TRANSACTION FEE",
					"COMMODITY_NAME2" => "TRANSACTION FEE");
					array_push($commodities, $fee);
			}else{
				$fee= array("COMMODITY_ID" => "aa",
					"COMMODITY_PRICE" => number_format($productprice-($veritrans->gross_amount),0,'',''),
					"COMMODITY_QTY" => 1,
					"COMMODITY_NAME1" => "TRANSACTION FEE",
					"COMMODITY_NAME2" => "TRANSACTION FEE");
					array_push($commodities, $fee);
